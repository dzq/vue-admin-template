<template>
  <div  class="app-container">
    <avue-crud ref="crud" :data="list" v-model="item" :option="option"
               :permission="permission" v-if="visible"
      ...
      <template slot="menuLeft">
        ...
        <el-button @click="getTree">获取菜单</el-button>
      ...
      <!--      </template>-->
      <!--  状态 编辑    -->
      <template slot-scope="scope" slot="parentForm">
        <el-cascader
      ...
      return {
        ...
        option: {
          ...
          column: [
            ...
            { label:"菜单标识符", prop:"name"},
            { label:"菜单路径", prop:"path"},
            { label:"菜单图标", prop:"icon"},
            { label:"父级", prop:"parent"},
    ...
    methods:{
      getTree(){
        this.api.getTree().then(r=>{
          ...
          this.code = JSON.stringify(r.data ,null,"\t");
        })
      },
      parseItem(item,parent=0,fullPath){

        const {id,path,name,title,icon} = item
        fullPath = parent==0?[0]:[...fullPath]
        fullPath.push(id)
        let menu = {id,parent,path,name,title,fullPath,icon}
        // console.log(menu)
